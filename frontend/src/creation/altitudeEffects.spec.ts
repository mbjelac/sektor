...
describe("propertiesShapedByAltitude", () => {
  ...
  // The middle location is overlooked from all four sides, each corner from two, and the ground
  // standing over them is in nobody's shadow.
  ...
  // It takes no cliff to stand over a location: ground a single step up casts its shadow like any
  // other, and the same locations are dimmed by the same amount as by ground standing far higher.
  it("dims insolation for a neighbour standing a single step higher", () => {
    const insolation = [[12, 12, 12], [12, 12, 12], [12, 12, 12]];
